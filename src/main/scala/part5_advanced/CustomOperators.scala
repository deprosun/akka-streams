...
object CustomOperators extends App {
  ...
  class RandomNumberGenerator(max: Int) extends GraphStage[ /*step 0: define the shape*/ SourceShape[Int]] {
    ...
    val outPort = Outlet[Int]("randomNumberGenerator")
    ...
    override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape) {
      ...
      setHandler(inPort, new InHandler {
        ...
        override def onPush(): Unit = {
          ...
          // back pressure happens automatically
          Thread.sleep(100)
  ...
  /**
    * Exercise: a custom flow - simple filter flow
    * - 2 ports: an inout port and an output port
    */

  class SimpleFilter[T](p: T => Boolean) extends GraphStage[FlowShape[T, T]] {
    val inPort = Inlet[T]("simpleFilterIn")
    val outPort = Outlet[T]("simpleFilterOut")
      ...
      setHandler(inPort, new InHandler {
        override def onPush(): Unit = {
          try {
            ...
            if (p(elem)) push(outPort, elem) // pass it on
  ...
  /**
    * Materialzied values in graph stages
    */
  ...
  val countFuture = Source(1 to 10)
    ...
    .to(Sink.foreach(x => if (x == 7) throw new RuntimeException("gotcha!") else println(x)))
    .run()

  import system.dispatcher

  countFuture.onComplete {
    case Success(c) => println(s"Number of elements passed ${c}")
    case Failure(ex) => println(s"counter failed $ex")
  }
