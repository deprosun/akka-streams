...
class TestingStreamSpec extends TestKit(ActorSystem("TestingAkkaStreams"))
  ...
  "A simple stream" should {
    ...
    "integrate with Streams TestKit Sink" in {
      ...
      val materialziedTestValue = sourceUnderTest.runWith(testSink)

      materialziedTestValue
        .request(5)
    ...
    "integrate with Streams TestKit Source" in {
      ...
      resultFuture.onComplete {
        case Success(_) => fail("the sink under test should have thrown exception")
        case Failure(_) => // ok
      }
    }
    ...
    "test flows with a test source AND a test sink" in {
      ...
      val materializedValue = testSource
        .via(flowUnderTest)
